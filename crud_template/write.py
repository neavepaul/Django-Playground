...
def write_instructors():
    # User created directly from the Instructor class as Instructor inherits from User
    instructor_neave = Instructor(first_name='Neave', last_name='Paul', dob=date(2002, 8, 3), full_time=True, total_learners=30050)
    instructor_neave.save()
    
    instructor_hudah = Instructor(first_name='Hudah', last_name='Ansari', dob=date(2002, 7, 16), full_time=True, total_learners=30050)
    instructor_hudah.save()

    instructor_afif = Instructor(first_name='Afif', last_name='Deshmukh', dob=date(1992, 1, 2), full_time=False, total_learners=10040)
    instructor_afif.save()

    instructor_john = Instructor(first_name='John', last_name='Doe', dob=date(1982, 5, 2), full_time=True, total_learners=2002)
    instructor_john.save()
    print("Instructor objects all saved... ")

def write_learners():
    # User created directly from the Instructor class as Learner inherits from User
